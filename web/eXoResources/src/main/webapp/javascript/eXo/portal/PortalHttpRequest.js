...
/*******************************************************************************
 * Portal Ajax Response Data Structure {PortalResponse} | |--->{PortletResponse} |
 * |--->{PortletResponse} | |-->{portletId} | | | |-->{PortletResponseData} | | |
 * |--->{BlockToUpdate} | | |-->{BlockToUpdateId} | | |-->{BlockToUpdateData} | | |
 * |--->{BlockToUpdate} | |--->{PortalResponseData} | | | |--->{BlockToUpdate} | |
 * |-->{BlockToUpdateId} | | |-->{BlockToUpdateData} | | | |--->{BlockToUpdate}
 * |--->{PortalResponseScript}
 * 
 ******************************************************************************/

/*
 * This object is wrapper on the value of each HTML block returned by an eXo
 * Portal AJAX call.
 * 
 * This includes: - the portle ID - the portlet title - the portlet mode - the
 * portlet state - the portlet content - the updated scripts to dynamically load
 * in the browser
 * 
 * Then each block to update within the portlet are place in a object which is
 * itself placed inside an array to provide an OO view of the AJAX response
 */
...
(function($, base, uiMaskLayer, msg) {
  function PortletResponse(responseDiv) {
    ...
    if (blocks.length > 0) {
      ...
      /*
       * If there is no block to update it means we are in a JSR 286 / 168
       * portlet In that case we need to find all the script tags and
       * dynamically execute them.
       * 
       * Indeed, when being in an AJAX call that return some <script> tag, local
       * functions are lost when calling the eval() methods. When the code is
       * written by eXo we use global scoped funstions like "instance.myFunction =
       * function(arguments)".
       * 
       * But when the code is provided by a third party portlet, it is not
       * possible to force that good practise. Hence we have to dynamically
       * reference the embedded script in the head tag
       */
  ...
  /** ************************************************************************************** */
  /*
   * This object is an OO wrapper on top of the returning HTML included in the
   * PortalResponse tag.
   * 
   * It allows to split in two different arrays the portletResponse blocks and
   * the one and the PortalResponseData one
   * 
   * It also extract from the HTML the javascripts script to then be dynamically
   * evaluated
   */
  function PortalResponse(responseDiv) {
    ...
    // Portlet Response
    div
        .children("div.PortletResponse")
        .each(
            function() {
              portalResp.portletResponses[portalResp.portletResponses.length] = new PortletResponse(
                  this);
            });
    // Portal Response
    div.children("div.PortalResponseData").each(function() {
      ...
      $(this).children(".BlockToUpdate").each(function() {
        ...
        portalResp.blocksToUpdate[i] = {
          blockId : dataBlocks[0].innerHTML,
          data : dataBlocks[1]
        };
         ...
         * 
         * This is needed when we refresh an entire portal page that contains
         * some standard JSR 168 / 286 portlets with embeded <script> tag
         */
  ...
  /*
   * This function is used to dynamically append a script to the head tag of the
   * page
   */
  ...
  /** ************************************************************************************** */
  /*
   * This is the main object that acts both as a field wrapper and a some status
   * method wrapper
   * 
   * It is also the object that has the reference to the XHR request thanks to a
   * reference to the eXo.core.Browser object
   */
  function AjaxRequest(method, url, queryString) {
    ...
    /*
     * This method is executed only if the boolean "onLoadingInternalHandled" is
     * set to false The method delegate the call to the ajaxLoading() method of
     * the HttpResponseHandler
     */
    instance.onLoadingInternal = function() {
      if (instance.onLoadingInternalHandled)
        return;

      if (typeof (instance.onLoading) == "function")
        instance.onLoading(instance);
    ...
    /*
     * This method is executed only if the boolean "onCompleteInternalHandled"
     * is set to false The method delegate the call to the ajaxResponse() method
     * of the HttpResponseHandler after calling the onSuccess() method of the
     * current object
     * 
     * During the processof this method, all the instance fields are filled with
     * the content coming back from the AJAX call. Once the ajaxResponse() is
     * called then the callback object is called if not null
     */
    instance.onCompleteInternal = function(xhr, statusText) {
      if (instance.onCompleteInternalHandled || instance.aborted)
        return;
      ...
      if (statusText == "timeout") {
        ...
        if (statusText == "success"
            && typeof (instance.onSuccess) == "function") {
          instance.onSuccess(instance);
          instance.onCompleteInternalHandled = true;
          if (typeof (instance.callBack) == "function") {
            instance.callBack(instance);
          } else if (instance.callBack) { // Modified by Uoc Nguyen: allow user
                                          // use custom javascript code for
                                          // callback
            try {
    ...
    /*
     * This method is executed only if the boolean "onLoadingInternalHandled" is
     * set to false The method delegate the call to the ajaxTimeout() method of
     * the HttpResponseHandler
     */
    instance.onTimeoutInternal = function() {
      if (instance == null || instance.request == null
          || instance.onCompleteInternalHandled)
        return;
    ...
    /*
     * This method is directly called from the doRequest() method. It opens a
     * connection to the server, set up the handlers and sends the query to it.
     * Status methods are then called on the request object during the entire
     * lifecycle of the call
     * 
     * It also sets up the time out and its call back to the method of the
     * current instance onTimeoutInternal()
     */
   ...
   * executeScript - updateBlocks - ajaxTimeout - ajaxResponse - ajaxLoading
   * 
  ...
  function HttpResponseHandler() {
    var instance = new Object();
    /*
     * instance.to stores a timeout object used to postpone the display of the
     * loading popup the timeout is defined later in the instance.ajaxLoading
     * function
     */
    instance.to = null;
    /*
      ...
      if (response) {
        ...
        if (portletResponses) {
          for (var i = 0; i < portletResponses.length; i++) {
            head.find(
                ".ExHead-" + portletResponses[i].portletId + ":not(title)")
                .remove();
          }
        }

        if (response.data) {
          $(response.data).find(".PORTLET-FRAGMENT").each(
              function() {
                head.find(
                    ".ExHead-" + this.parentNode.id.replace("EditMode-", "")
                        + ":not(title)").remove();
              });
        }
      } else {
    ...
    /*
     * This methods will replace some block content by new one. This is the
     * important concept in any AJAX call where JS is used to dynamically
     * refresh a part of the page.
     * 
     * The first argument is an array of blocks to update while the second
     * argument is the id of the html component that is the parent of the block
     * to update
     * 
     * Each block in the array contains the exact id to update, hence a loop is
     * executed for each block and the HTML is then dynamically replaced by the
     * new one
     */
     ...
     * request.responseText has been filled with the returning HTML. Hence the
     * goal of this method is to update the diffent blocks dynamically.
     * 
     * 1) Create a temporary div element and set the response HTML text to its
     * innerHTML variable of the temp object 2) Use the
     * DOMUtil.findFirstDescendantByClass() method to get the div with the Id
     * "PortalResponse" out of the returned HTML 3) Create the PortalResponse
     * object by passing the previous DOM element as an argumen, it will provide
     * an OO view of the PortletResponse and other portal response blocks to
     * update 4) Each portlet response block is the updated using the naming
     * convention "UIPortlet-" + portletId; and then the script are loaded 5)
     * Then it is each portal block which is updated and the assocaited scripts
     * are evaluated
     */
    instance.ajaxResponse = function(request, response) {
      ...
      var portletResponses = response.portletResponses;
      if (portletResponses) {
        $.each(portletResponses,
            function() {
              var portletResponse = this;
              if (!portletResponse.blocksToUpdate) {
                /*
                 * This means that the entire portlet fragment is included in
                 * the portletResponse.portletData and that it does not contain
                ...
                if (portletResponse.scripts) {
                  portletResponse.scripts.each(function(index) {
                    var encodedName = 'script_' + index + '_'
                        + portletResponse.portletId;
      ...
      if (!response.blocksToUpdate && request.responseText !== "") {
        if (confirm(msg.getMessage("SessionTimeout")))
          instance.ajaxTimeout(request);
      }
      try {
        // Handle the portal responses
        instance.updateBlocks(response.blocksToUpdate);
        // After handle html response. We need to remove extra markup header of
        // removed portlets
        cleanHtmlHead();
        instance.executeScript(response.script);
        /**
         * Clears the instance.to timeout if the request takes less time than
         * expected to get response Removes the transparent mask so the UI is
         * available again, with cursor "auto"
         */
    ...
    /*
     * This method is called when doing an AJAX call, it will put the "Loading"
     * image in the middle of the page for the entire call of the request
     */
    instance.ajaxLoading = function(request) {
      if (request.isAsynchronize())
        return;
      /**
       * Waits 2 seconds (2000 ms) to display the loading popup if the response
       * comes before this timeout, the loading popup won't appear at all
       * Displays a transparent mask with the "wait" cursor to tell the user
  ...
  /*
   * The doRequest() method takes incoming request from GET and POST calls The
   * second argument is the URL to target on the server The third argument is
   * the query string object which is created out of a form element, this value
   * is not null only when there is a POST request.
   * 
   * 1) An AjaxRequest object is instanciated, it holds the reference to the XHR
   * method 2) An HttpResponseHandler object is instantiated and its methods
   * like ajaxResponse, ajaxLoading, ajaxTimeout are associated with the one
   * from the AjaxRequest and will be called by the XHR during the process
   * method
   */
